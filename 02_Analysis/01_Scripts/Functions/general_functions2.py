...
def create_balanced_mask(ditchArr, height, width):
    ...
    max_height, max_width = ditchArr.shape  # Get array dimensions

    for i in range(0, max_height, height):
        for j in range(0, max_width, width):
            ...
            for k in range(height):
                for l in range(width):
                    if i + k < max_height and j + l < max_width:  # ✅ Prevent index overflow
            ...
            if zoneContainsDitches:
                for m in range(height):
                    for n in range(width):
                        if i + m < max_height and j + n < max_width:  # ✅ Prevent overflow
                            new_arr[i+m, j+n] = 1
            else:
                for m in range(height):
                    for n in range(width):
...
def create_balanced_dataset(list_of_zones):
    ...
    for i, zone_file_name in enumerate(list_of_zones):
        ...
        mask = create_balanced_mask(zone["label_3m"].values.reshape((5000,5000)), 3, 5)
